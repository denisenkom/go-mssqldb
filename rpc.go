...
func sendRpc(buf *tdsBuffer, headers []headerStruct, proc ProcId, flags uint16, params []Param) (err error) {
    ...
    if len(proc.name) == 0 {
        ...
        err = binary.Write(buf, binary.LittleEndian, &idswitch); if err != nil {
            return
        }
        err = binary.Write(buf, binary.LittleEndian, &proc.id); if err != nil {
            return
        }
    } else {
        err = writeUsVarChar(buf, proc.name); if err != nil {
            return
        }
    }
