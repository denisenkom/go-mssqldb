...
func TestSelect(t *testing.T) {
	...
	values := []testStruct{
		...
		{"cast(cast(N'проверка' as nvarchar(max)) collate Cyrillic_General_CI_AI as varchar(max))", "проверка"},
		{"cast(N'Δοκιμή' as nvarchar(max))", "Δοκιμή"},
		{"cast(cast(N'Δοκιμή' as nvarchar(max)) collate Greek_CI_AI as varchar(max))", "Δοκιμή"},
